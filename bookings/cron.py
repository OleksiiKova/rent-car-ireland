...
class UpdateBookingStatusCronJob(CronJobBase):
    ...
    This job runs periodically and updates the status of bookings where the end date 
    is earlier than the current date and the status is still 'confirmed'. The status
    of such bookings is changed to 'completed'.
